...
// Input pump (Langkah 9): SDL event -> Input_Event -> VT bytes -> PTY write,
// plus window-resize -> winsize + terminal_resize.
//
...
// src/app/main.odin (the renderer-owned metrics replace them in Langkah 13;
...
//   KEYDOWN special keys -> mapped kinds with live Shift/Alt/Ctrl flags:
//     arrows, Home, End, PgUp, PgDn, Enter (K_RETURN, K_RETURN2,
//     K_KP_ENTER), Backspace, Tab (K_TAB, K_LEFT_TAB), Escape.
...
_translate_key :: proc(key: sdl3.Keycode, mod: sdl3.Keymod, out: []Input_Event) -> bool {
	...
	switch key {
	case sdl3.K_UP:
		out[0] = Input_Event{kind = .Arrow_Up, shift = shift, alt = alt, ctrl = ctrl}
		return true
	case sdl3.K_DOWN:
		out[0] = Input_Event{kind = .Arrow_Down, shift = shift, alt = alt, ctrl = ctrl}
		return true
	case sdl3.K_LEFT:
		out[0] = Input_Event{kind = .Arrow_Left, shift = shift, alt = alt, ctrl = ctrl}
		return true
	case sdl3.K_RIGHT:
		out[0] = Input_Event{kind = .Arrow_Right, shift = shift, alt = alt, ctrl = ctrl}
		return true
	case sdl3.K_HOME:
		out[0] = Input_Event{kind = .Home, shift = shift, alt = alt, ctrl = ctrl}
		return true
	case sdl3.K_END:
		out[0] = Input_Event{kind = .End, shift = shift, alt = alt, ctrl = ctrl}
		return true
	case sdl3.K_PAGEUP:
		out[0] = Input_Event{kind = .PgUp, shift = shift, alt = alt, ctrl = ctrl}
		return true
	case sdl3.K_PAGEDOWN:
		out[0] = Input_Event{kind = .PgDn, shift = shift, alt = alt, ctrl = ctrl}
		return true
	...
	case sdl3.K_BACKSPACE:
		...
		out[0] = Input_Event{kind = .Backspace, shift = shift, alt = alt, ctrl = ctrl}
		return true
	case sdl3.K_TAB, sdl3.K_LEFT_TAB:
		...
		out[0] = Input_Event{kind = .Tab, shift = shift, alt = alt, ctrl = ctrl}
		return true
	case sdl3.K_ESCAPE:
		if alt && !ctrl {
	...
	case:
		if ctrl {
			...
			if r > 0 && r <= utf8.MAX_RUNE {
				out[0] = Input_Event{kind = .Ctrl, rune = r, ctrl = true, alt = alt, shift = shift}
				return true
			}
			return false
		}
...
input_pump :: proc(w: ^win.Window, p: ^pty.Pty, t: ^termgrid.Terminal) -> (quit: bool, resized: bool, ok: bool) {
	if w == nil {
	...
	if (w.pixel_w != prev_pw || w.pixel_h != prev_ph) && t != nil {
		r, wins_ok := input_pump_resize(p, t, int(w.pixel_w), int(w.pixel_h))
		resized = r
		ok = ok && wins_ok
	}
